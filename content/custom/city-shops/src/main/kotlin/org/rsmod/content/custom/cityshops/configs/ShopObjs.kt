...
object ShopObjs : ObjReferences() {
    ...
    val wizards_robe = find("wizards_robe")
    val wooden_shield = find("wooden_shield")
    // Metal bolts are not `iron_bolts` and friends in this cache -- `bolt`/`bolts` are the bronze
    // pair, and every metal tier lives under the crossbow-quest prefix. Note `adamantite` and
    // `runite` rather than `adamant` and `rune`.
