...
class TertiaryDrop(val oneIn: Int, val drop: DropQuantity, val rare: Boolean = false)
 ...
 *
 * [rare] marks a slot that the 16x experience tier boosts. It is declared explicitly rather than
 * inferred from the odds: a weight-based threshold would be relative to the table's denominator, so
 * a purely cosmetic rebalance could silently reclassify a slot, and nested tables make "the"
 * probability ambiguous anyway - local, or multiplied by the chance of reaching the sub-table?
 */
sealed class DropSlot {
    abstract val weight: Int

    abstract val rare: Boolean

    /** Drops [drop] when rolled. */
    class Item(
        override val weight: Int,
        val drop: DropQuantity,
        override val rare: Boolean = false,
    ) : DropSlot()
...
class DropTableBuilder {
    ...
    /** As [tertiary], but flagged rare so the 16x experience tier boosts it. */
    fun rareTertiary(oneIn: Int, obj: ObjType, count: Int = 1) {
        tertiary += TertiaryDrop(oneIn, DropQuantity(obj, count..count), rare = true)
    }
...
class WeightedTableBuilder(private val denominator: Int) {
    ...
    /** As [drop], but flagged rare so the 16x experience tier boosts it. */
    fun rare(weight: Int, obj: ObjType, count: Int = 1) {
        rare(weight, obj, count..count)
    }

    fun rare(weight: Int, obj: ObjType, count: IntRange) {
        slots += DropSlot.Item(weight, DropQuantity(obj, count), rare = true)
    }

    /**
     * As [nested], but flagged rare. The boost applies both to the chance of reaching [table] and,
     * inherited downward, to every slot inside it - which is how the shared rare drop table should
     * be declared.
     */
    fun rareNested(weight: Int, table: WeightedTable) {
        slots += DropSlot.Nested(weight, table, rare = true)
    }
